...
f11$Stime_resc<- 10^((1/f11$z)*(f11$ctmax-f11$ta.mn)+ #log time at median Ta
...
m11$Stime_resc<- 10^((1/m11$z)*(m11$ctmax-m11$ta.mn)+ #log time at median Ta
...
mall$Stime_resc<- 10^((1/mall$z)*(mall$ctmax-mall$ta.mn)+ #log time at median Ta
                        log10(data_m$surv.time2)- #log time observed
                        (1/mall$z)*(mall$ctmax-data_m$test.temp)) #log time expected
#-------------------------------------------------------------------------------
# FIGURE 1
#-------------------------------------------------------------------------------
{
  #pdf("../Outputs/1.2.1. Figure 1 Survival probabilities of DGRP lines.pdf",width = 6,height = 6,useDingbats = FALSE)
  png("../Outputs/1.2.1. Figure 1 Survival probabilities of DGRP lines.png",width = 8,height = 8,units = "in",res = 600)
  ...
  text(65,92,"Inbred lines",col="gray",adj=0,font=2,cex=1.2)
  text(65,85,"Inbred lines pooled",adj=0,col="#4DAC26",font=2,cex=1.2)
  #-----------------------------------------------------------------------------
  #males
  # labels
  labs= c("1","2","5","10","30","120","420")
  ...
  plot(surv.time ~ jitter(test.temp,0.5),xlim=c(35.6,39.4),ylim=c(0,3),yaxt="n",
       ...
       xlab="Test temperature (ºC)",ylab="Time (min)",data=m.25180)
  ...
  text(65,92,"Inbred lines",col="gray",adj=0,font=2,cex=1.2)
  text(65,85,"Inbred lines pooled lines",adj=0,col="#A6761D",font=2,cex=1.2)
...
write.csv(df.f20,"../Outputs/1.1.1. Rescaled survival probability of DGRP lines females.csv",row.names = FALSE)
write.csv(df.m20,"../Outputs/1.1.2. Rescaled survival probability of DGRP lines males.csv", row.names = FALSE)

#saving session information with all packages versions for reproducibility
#purposes
sink("../Outputs/1.1.3. Survival probability of DGRP lines_R_session.txt")
sessionInfo()
sink()
################################################################################
############################ END OF SCRIPT #####################################
